...
sub main {
    ...
    chdir "/opt";
    my $dirname ="/opt/vfabric-insight-dashboard";
    if (-d $dirname) {
        rmtree $dirname;
    }
    ...
    mkdir $dirname, 0755;
    chdir($dirname);
    my $file_name = "vfabric-tc-server-standard-2.9.4.CI-30.zip";
    my $file_name1 = "insight-vfabric-tc-server-dashboard-1.9.2.SR3.zip";
    #my $file_name2="insight-vfabric-tc-server-agent-1.9.2-CI-20130131.023014-1.zip";
    my $dirname1="$dirname/vfabric-tc-server-standard-2.9.4.CI-30";
   # my $dirname2="$dirname/insight-agent";
    ...
    runCommand('wget --no-check-certificate "https://dist.springsource.com/snapshot/TCS/vfabric-tc-server-standard-2.9.4.CI-30.zip"');
    runCommand("unzip $dirname/$file_name");
    runCommand('wget --no-check-certificate "https://private.maven.springsource.com/release/com/springsource/insight/dist/insight-vfabric-tc-server-dashboard/1.9.2.SR3/insight-vfabric-tc-server-dashboard-1.9.2.SR3.zip"');
    ...
    runCommand("mv  $dirname3 $dirname1/templates");
    chdir($dirname1);
    runCommand("$pwd");
