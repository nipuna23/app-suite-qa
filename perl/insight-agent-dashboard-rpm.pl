...
#    my $stat=runCommand("netstat -na | egrep '8080|9090' | grep LISTEN");
    ...
    my $dirname ="/root/perl/testdir";
    if (-d $dirname) {
        rmtree $dirname;
    }
    mkdir $dirname, 0755;
    chdir($dirname);
    my $file_name = "vfabric-tc-server-standard-2.8.2.RELEASE.zip";
    my $file_name1 = "insight-vfabric-tc-server-dashboard-1.9.2-CI-20130130.022145-127.zip";
    my $file_name2="insight-vfabric-tc-server-agent-1.9.2-CI-20130131.023014-1.zip";
    my $dirname1="$dirname/vfabric-tc-server-standard-2.8.2.RELEASE";
    ...
    runCommand("rpm -qa | grep vfabric");
    runCommand("rpm -e vfabric-5.3-repo-5.3-1.noarch ");
    runCommand('wget -q -O - http://repo.vmware.com/pub/rhel6/vfabric/5.3/vfabric-5.3-silent-suite-installer | sh');   
   # runCommand("yum search vfabric");
    runCommand("yum install vfabric-insight-dashboard.noarch");
    sleep 20;
exit;
    runCommand('wget --no-check-certificate "https://private.maven.springsource.com/snapshot/com/springsource/insight/dist/insight-vfabric-tc-server-dashboard/1.9.2-CI-SNAPSHOT/insight-vfabric-tc-server-dashboard-1.9.2-CI-20130130.022145-127.zip"');
    ...
    runCommand("mv $dirname2 $dirname3 $dirname1/templates");
    chdir($dirname1);
    runCommand("$pwd");
